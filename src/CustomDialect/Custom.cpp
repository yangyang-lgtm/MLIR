...
#include "CustomDialect/CustomDialect.cpp.inc"
...
void CustomDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "CustomDialect/Custom.cpp.inc"
      >();
  addTypes<
#define GET_TYPEDEF_LIST
#include "CustomDialect/CustomTypes.cpp.inc"
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include "CustomDialect/CustomAttributes.cpp.inc"
      >();
}

/// Materialize a single constant operation from a given attribute value with
/// the desired resultant type.
Operation *CustomDialect::materializeConstant(OpBuilder &builder,
                                             Attribute value, Type type,
                                             Location loc) {
  return builder.create<custom::ConstantOp>(loc, type, value);
}

/// Default callback for builders of ops carrying a region. Inserts a yield
/// without arguments.
void mlir::custom::buildTerminatedBody(OpBuilder &builder, Location loc) {
  builder.create<custom::YieldOp>(loc);
}
...
bool mlir::custom::isSupportedCustomType(Type type) {
  ...
  if (auto tensorType = llvm::dyn_cast<TensorType>(type)) {
    ...
    if (llvm::isa<custom::ArrayType>(elemType)) {
      return false;
    }
    return isSupportedCustomType(elemType);
  }
  if (auto tupleType = llvm::dyn_cast<TupleType>(type)) {
    return llvm::all_of(tupleType.getTypes(), [](Type type) {
      return !llvm::isa<custom::ArrayType>(type) && isSupportedCustomType(type);
    });
  }
  return false;
}

bool mlir::custom::isSupportedIntegerType(Type type) {
...
bool mlir::custom::isIntegerIndexOrOpaqueType(Type type) {
  return llvm::isa<IndexType, custom::OpaqueType>(type) ||
         isSupportedIntegerType(type) || isPointerWideType(type);
}

bool mlir::custom::isSupportedFloatType(Type type) {
...
bool mlir::custom::isPointerWideType(Type type) {
  return isa<custom::SignedSizeTType, custom::SizeTType, custom::PtrDiffTType>(
      type);
}
...
static LogicalResult verifyInitializationAttribute(Operation *op,
  ...
  if (llvm::isa<custom::OpaqueAttr>(value))
    return success();

  if (llvm::isa<StringAttr>(value))
    return op->emitOpError()
           << "string attributes are not supported, use #custom.opaque instead";
  ...
  if (resultType != attrType)
    return op->emitOpError()
           << "requires attribute to either be an #custom.opaque attribute or "
              "it's type ("
...
LogicalResult AddOp::verify() {
  ...
  if (isa<custom::PointerType>(lhsType) && isa<custom::PointerType>(rhsType))
    return emitOpError("requires that at most one operand is a pointer");

  if ((isa<custom::PointerType>(lhsType) &&
       !isa<IntegerType, custom::OpaqueType>(rhsType)) ||
      (isa<custom::PointerType>(rhsType) &&
       !isa<IntegerType, custom::OpaqueType>(lhsType)))
...
bool CastOp::areCastCompatible(TypeRange inputs, TypeRange outputs) {
  ...
  if (auto arrayType = dyn_cast<custom::ArrayType>(input)) {
    if (auto pointerType = dyn_cast<custom::PointerType>(output)) {
  ...
  return (
      (custom::isIntegerIndexOrOpaqueType(input) ||
       custom::isSupportedFloatType(input) || isa<custom::PointerType>(input)) &&
      (custom::isIntegerIndexOrOpaqueType(output) ||
       custom::isSupportedFloatType(output) || isa<custom::PointerType>(output)));
}
...
LogicalResult custom::CallOpaqueOp::verify() {
  ...
  if (std::optional<ArrayAttr> templateArgsAttr = getTemplateArgs()) {
    for (Attribute tArg : *templateArgsAttr) {
      if (!llvm::isa<TypeAttr, IntegerAttr, FloatAttr, custom::OpaqueAttr>(tArg))
...
LogicalResult custom::ConstantOp::verify() {
  Attribute value = getValueAttr();
  if (failed(verifyInitializationAttribute(getOperation(), value)))
    return failure();
  if (auto opaqueValue = llvm::dyn_cast<custom::OpaqueAttr>(value)) {
...
OpFoldResult custom::ConstantOp::fold(FoldAdaptor adaptor) { return getValue(); }
...
LogicalResult ExpressionOp::verify() {
  ...
  for (Operation &op : region.front().without_terminator()) {
    if (!op.hasTrait<OpTrait::custom::CExpression>())
...
LogicalResult custom::LiteralOp::verify() {
  if (getValue().empty())
    return emitOpError() << "value must not be empty";
  return success();
}
...
LogicalResult SubOp::verify() {
  ...
  if (isa<custom::PointerType>(rhsType) && !isa<custom::PointerType>(lhsType))
    return emitOpError("rhs can only be a pointer if lhs is a pointer");

  if (isa<custom::PointerType>(lhsType) &&
      !isa<IntegerType, custom::OpaqueType, custom::PointerType>(rhsType))
    return emitOpError("requires that rhs is an integer, pointer or of opaque "
                       "type if lhs is a pointer");

  if (isa<custom::PointerType>(lhsType) && isa<custom::PointerType>(rhsType) &&
      !isa<IntegerType, custom::PtrDiffTType, custom::OpaqueType>(resultType))
...
LogicalResult custom::VariableOp::verify() {
...
LogicalResult custom::YieldOp::verify() {
...
LogicalResult custom::SubscriptOp::verify() {
  // Checks for array operand.
  if (auto arrayType = llvm::dyn_cast<custom::ArrayType>(getValue().getType())) {
  ...
  if (auto pointerType =
          llvm::dyn_cast<custom::PointerType>(getValue().getType())) {
    // Check number of indices.
...
LogicalResult custom::VerbatimOp::verify() {
...
FailureOr<SmallVector<ReplacementItem>> custom::VerbatimOp::parseFormatString() {
...
#include "CustomDialect/CustomEnums.cpp.inc"
...
Type custom::ArrayType::parse(AsmParser &parser) {
  if (parser.parseLess())
    return Type();
...
void custom::ArrayType::print(AsmPrinter &printer) const {
  printer << "<";
...
LogicalResult custom::ArrayType::verify(
...
custom::ArrayType
custom::ArrayType::cloneWith(std::optional<ArrayRef<int64_t>> shape,
...
LogicalResult mlir::custom::LValueType::verify(
    llvm::function_ref<mlir::InFlightDiagnostic()> emitError,
    mlir::Type value) {
  // Check that the wrapped type is valid. This especially forbids nested
  // lvalue types.
  if (!isSupportedCustomType(value))
    return emitError()
           << "!custom.lvalue must wrap supported custom type, but got " << value;

  if (llvm::isa<custom::ArrayType>(value))
    return emitError() << "!custom.lvalue cannot wrap !custom.array type";
...
LogicalResult mlir::custom::OpaqueType::verify(
    llvm::function_ref<mlir::InFlightDiagnostic()> emitError,
    llvm::StringRef value) {
  if (value.empty()) {
    return emitError() << "expected non empty string in !custom.opaque type";
  }
  if (value.back() == '*') {
    return emitError() << "pointer not allowed as outer type with "
                          "!custom.opaque, use !custom.ptr instead";
  }
  return success();
}
...
LogicalResult mlir::custom::PointerType::verify(
    llvm::function_ref<mlir::InFlightDiagnostic()> emitError, Type value) {
  if (llvm::isa<custom::LValueType>(value))
...
// GlobalOp
//===----------------------------------------------------------------------===//
static void printCustomGlobalOpTypeAndInitialValue(OpAsmPrinter &p, GlobalOp op,
                                                  TypeAttr type,
...
static ParseResult
parseCustomGlobalOpTypeAndInitialValue(OpAsmParser &parser, TypeAttr &typeAttr,
                                      Attribute &initialValue) {
  Type type;
  ...
  if (!llvm::isa<ElementsAttr, IntegerAttr, FloatAttr, custom::OpaqueAttr>(
          initialValue))
    return parser.emitError(parser.getNameLoc())
...
LogicalResult GlobalOp::verify() {
  if (!isSupportedCustomType(getType())) {
    return emitOpError("expected valid custom type");
  }
  if (getInitialValue().has_value()) {
    ...
    } else if (!isa<custom::OpaqueAttr>(initValue)) {
...
GetGlobalOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (!global)
    return emitOpError("'")
           << getName() << "' does not reference a valid custom.global";
...
static LogicalResult verifyRegion(custom::SwitchOp op, Region &region,
                                  const Twine &name) {
  auto yield = dyn_cast<custom::YieldOp>(region.front().back());
  if (!yield)
    return op.emitOpError("expected region to end with custom.yield, but got ")
...
LogicalResult custom::SwitchOp::verify() {
...
unsigned custom::SwitchOp::getNumCases() { return getCases().size(); }

Block &custom::SwitchOp::getDefaultBlock() { return getDefaultRegion().front(); }

Block &custom::SwitchOp::getCaseBlock(unsigned idx) {
